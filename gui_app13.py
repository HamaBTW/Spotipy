...
class Session(ft.UserControl):
    ...
    def GoToSongs(self, data):
        self.page.clean()
        app = SongsList(data, self.page, self.curnt_app_version)
...
class MyApp(ft.UserControl):
    ...
    def build(self):
        ...
        #search zone
        self.search_field_text = ft.TextField(label="Search", on_change=self.search_c, expand=True, animate_scale=20)
...
class SongsList(ft.UserControl):
    ...
    def build(self):
        ...
        self.load_btn = ft.IconButton(
            icon=ft.icons.KEYBOARD_ARROW_UP, on_click=self.reload,
        )
        ...
        #search zone
        self.search_field = ft.TextField(label="Search", on_change=self.search_c, expand=True,)
        self.search_icon = ft.Icon(ft.icons.SEARCH, opacity=0.5, expand=True,)
        self.search_drop_down = ft.Dropdown(
            value="Song - Artist",
            options=[
    ...
    def GoToHome1(self, data):
        self.page.clean()
        self.page.add(MyApp(self.page, self.curnt_app_version))
        self.page.update()
        print("ss")
        #app.Load_data(None)
        MyApp(self.page, self.curnt_app_version).Load_data(None)
        print("rrr")
        self.page.update()

    def GoToHome(self, data):
        self.page.clean()
        app = MyApp(self.page, self.curnt_app_version)
...
#flet pack gui_app13.py --name Spotipy --icon favicon.png
